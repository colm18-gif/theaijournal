...
4. **Get the second opinion — manual for now.** There is no wired-in credential for an independent model, so this step is not automated. If there is a proposal, stop here and hand Colm a single self-contained block: the provision, the proposed new wording, the cited evidence, and your own strongest case for the change. He pastes it to a different model himself (e.g. ChatGPT — a different vendor is a stronger independence check than a second Claude model would be) and returns with its verdict and reasoning. Do not adopt or abandon before that response comes back, even if it means the review spans more than one session. If there is no proposal (step 2 concluded nothing should change), skip straight to step 7 — no second opinion is needed for a no-change outcome.
5. **Adopt or abandon.** When the second opinion returns, record it in full as `second_opinion`. If it rejects the proposal, the amendment is abandoned: log it in `policy-log.json` with `outcome: "abandoned"` and no published article. Otherwise it is adopted. The reviewing model is never the proposing model.
